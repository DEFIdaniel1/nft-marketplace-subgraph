...
export class NFTPurchased extends Entity {
  ...
  save(): void {
    ...
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type NFTPurchased must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("NFTPurchased", id.toString(), this);
    }
  }

  static load(id: string): NFTPurchased | null {
    return changetype<NFTPurchased | null>(store.get("NFTPurchased", id));
  }
